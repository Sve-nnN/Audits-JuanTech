...
Planeando el próximo milestone (post v1.6).

## Prior State: v1.6 shipped 2026-08-06

Séptimo milestone entregado. Sexta categoría de score "Meta Tags / Social" (peso 0.10, rebalanceado desde on-page y datos estructurados) con 8 checks nuevos de Open Graph/Twitter Card/charset (`packages/meta-social`, motor puro desacoplado), validación de `og:image` con fetcher dedupeado y defensa SSRF propia, métricas de performance por página (response time/HTML size, independientes de PSI), y panel visual de preview social en el reporte (Google/Facebook/LinkedIn/X) con snippets HTML de fix prellenados y copiables. 21/24 requisitos con verificación formal `passed`; 3/24 (PAGEPERF-01..03) y el panel de preview (PREVIEW-01..04/FIX-01/02) quedan con código completo, testeado e integrado (0 blockers de integración, E2E trazado) pero con checkpoints de UAT/verificación humana diferidos — ver `STATE.md` → Deferred Verification. Integración cross-fase limpia (`gsd-integration-checker`, 0 blockers, 1 warning no bloqueante). Detalle completo en `.planning/MILESTONES.md` y `.planning/v1.6-MILESTONE-AUDIT.md`.

**Trabajo previsto posterior a v1.6:**
- Resolver los 2 checkpoints de verificación humana diferidos (Fase 28: smoke-test + thresholds FA-1/FA-2; Fase 32: 7 ítems visuales/de red real) — `/gsd-verify-work 28` y `/gsd-verify-work 32` cuando haya acceso real a `shared-postgres`.
- WR-05 (recomendaciones de CMS para `SOCIAL-01..08` sin mapear en `packages/cms-adapters`) — deuda técnica aceptada, backlog v1.7.
- SOCIAL-10..12, CMSFIX-08, IMG-05 — deferred requirements v1.6.x/v1.7 (ver `REQUIREMENTS.md` histórico archivado y `STATE.md` → Deferred Items).
...
- ✓ Response time + HTML size por página instrumentados en el crawl sin requests extra, con umbrales de severidad (warning/error) — v1.6 (PAGEPERF-01..03, Phase 28). Código completo, testeado e integrado (nada aguas abajo depende de esto, confirmado por integration checker). Falta smoke-test de re-crawl real + confirmación de Juan sobre los umbrales FA-1/FA-2 contra tráfico real — diferido (ver STATE.md → Deferred Verification)
- ✓ Panel de preview social en el reporte (Google/SERP, Facebook/LinkedIn 1.91:1 compartido, X/Twitter summary vs summary_large_image) + proxy server-side de imágenes con allowlist de origen (reusa defensa SSRF de Phase 31) + snippets HTML de fix prellenados con valores reales de la página, accesibles/copiables — v1.6 (PREVIEW-01..04, FIX-01/02, Phase 32). Verificado 4/4 must-haves con evidencia de código real, code review limpio (0 critical/warning tras 3 iteraciones de auto-fix: imageStatus mal atribuido a twitter:image, cap de 500 chars faltante, try/catch faltante en el proxy). 7 ítems de verificación visual/de red real + backstops geométricos quedan diferidos a UAT humano — diferido (ver STATE.md → Deferred Verification)

### Active

<!-- Current scope. Building toward these. -->

Sin milestone activo — v1.6 recién cerrado. Próximo milestone por definir (`/gsd-new-milestone`).
...
| `imageStatus` de `og:image` (IMG-01) NO se reusa ciegamente para `twitter:image` — sólo hereda el veredicto cuando ambas URLs son idénticas (comparadas en crudo, antes del cap de 500 chars) | Code review de Phase 32 (CR-01) encontró que `twitter:image` puede ser una URL declarada aparte y distinta; reusar el veredicto de `og:image` sin más mostraba "imagen no disponible" en X aunque la imagen real de Twitter estuviera perfectamente accesible. Una segunda vuelta de review (WR-01) encontró que la primera corrección comparaba las URLs ya truncadas, colapsando falsamente URLs largas distintas con el mismo prefijo de 500 chars — se corrigió comparando las URLs crudas vía una señal interna que nunca llega al modelo público | ✓ Good — v1.6 Phase 32, 3 iteraciones de auto-fix (commits 40f5963/7cfa49c/192dde0/9be317f), confirmado sin regresión por 3 pasadas de code review independientes |
| Allowlist de origin EXACTO en el proxy de imágenes del preview social (`apps/web/app/api/audits/[id]/preview-image/route.ts`), sin excepciones para CDN/subdominios | Decisión de seguridad bloqueada en `32-CONTEXT.md`: preferir falsos negativos (og:image servida desde CDN distinto al origin auditado devuelve 403 aunque IMG-01 ya la haya validado como alcanzable) antes que ampliar la superficie de ataque del proxy con reglas de excepción | — Pending revisión de producto (WR-03, no es un bug) — v1.6 Phase 32, decisión de Juan documentada en el contexto de fase, deliberadamente no tocada por el auto-fix loop |
...
---
*Last updated: 2026-08-06 — v1.6 Meta Tags / Social shipped (cierre de milestone)*
