...
- [x] **RENDER-01**: El auditor determina, sobre una muestra representativa, si cada página se renderiza server-side o client-side (compara HTML crudo vs DOM renderizado con Playwright) y lo reporta como issue.
- [x] **RENDER-02**: Cuando contenido clave (título, H1, texto principal) falta en el HTML crudo y solo aparece tras render, el auditor lo marca como riesgo SEO/AEO con severidad acorde (no lo trata como falla dura del score).
